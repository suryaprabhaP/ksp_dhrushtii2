...
export function exportEvidencePacketPDF(data) {
  ...
  doc.text('Generated by KSP DRISHTI - State Crime Records Bureau (SCRB) Automated Evidence Engine.', 15, 189);
...
export function exportChatExecutiveReportPDF({ messages, voiceLang = 'en-IN' }) {
  ...
  doc.setFontSize(15);
  doc.setTextColor(15, 23, 42);
  doc.text('KARNATAKA STATE POLICE • DRISHTI COMMAND', 105, 18, { align: 'center' });
  ...
  messages.slice(-8).forEach((m) => {
    ...
    doc.text(isBot ? `[KSP DRISHTI - ${m.agent_label || 'Command Agent'}]:` : `[INVESTIGATING OFFICER]:`, 15, currentY);
    currentY += 5;
  ...
  doc.save(`KSP_Drishti_Executive_Report_${Date.now()}.pdf`);
}
