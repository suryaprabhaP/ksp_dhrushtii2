...
def test_cloud_infrastructure():
    ...
    # ── 7. Zoho Catalyst Multimodal VLM (VL-Qwen3.6-35B-A3B) ────────────────
    t0 = time.time()
    try:
        from app.providers.zoho_vlm_provider import ZohoQuickMLVLMProvider
        vlm_prov = ZohoQuickMLVLMProvider()
        if vlm_prov.is_available():
            tiny_png = "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg=="
            ans, prov, metrics = vlm_prov.complete_vision("Ping visual test.", [tiny_png], max_tokens=10)
            elapsed = (time.time() - t0) * 1000
            log_result("Zoho Catalyst VLM", vlm_prov.endpoint_url, "POST", "PASS", f"VL-Qwen Active: '{ans.strip()[:25]}'", 200, elapsed)
        else:
            log_result("Zoho Catalyst VLM", "https://api.catalyst.zoho.in/quickml/v1", "POST", "WARN", "Zoho QuickML token not available", None, (time.time()-t0)*1000)
    except Exception as e:
        log_result("Zoho Catalyst VLM", "https://api.catalyst.zoho.in/quickml/v1", "POST", "WARN", f"VLM Notice: {str(e)[:50]} (Cascades gracefully)", None, (time.time()-t0)*1000)
